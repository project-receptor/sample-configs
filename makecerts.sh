...
for node in foo bar baz; do
  ...
  receptor --cert-signreq req=certs/testcert-$node.csr cacert=certs/ca.crt cakey=certs/ca.key outcert=certs/testcert-$node.crt verify=yes
done
